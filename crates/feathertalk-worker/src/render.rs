...
use crate::{
    CommandOutcome, RenderBackend, RenderJob, RenderSummary, RenderVariant, TaskReporter,
    check_max_output_frames, check_render_paths, checkpoint_descriptor, execution_name,
    is_inference_cancellation, project_task_error, render_job, render_task_error, render_variant,
    training_task_error};
    ...
    F: RawVideoSinkFactory + ?Sized,
{
    ...
    let rendered = execute_offline_render::<RenderBackend, M, R, ObservedSinkFactory<'_, F>>(
        model,
        device,
        &job.request,
        frame_reader,
    ...
    F: RawVideoSinkFactory + ?Sized,
{
    ...
    match variant {
        RenderVariant::OriginalUnet(configuration) => {
            let template = configuration.init(device);
            let restored = match load_training_checkpoint_model::<_>(
                &params.checkpoint,
                &template,
                device,
                &job.descriptor,
