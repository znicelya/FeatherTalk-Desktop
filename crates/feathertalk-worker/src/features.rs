...
#[derive(Debug)]
pub struct FeatureModel<B: Backend = CpuBackend> {
    encoder: BurnFeatherHubertEncoder<B>,
    model_sha256: String,
}

impl FeatureModel<CpuBackend> {
    pub fn load(features: &FeatureToolchain) -> Result<Self, PackageError> {
        Self::load_on(features, Default::default())
    }
}

impl<B: Backend> FeatureModel<B> {
    ...
    pub fn load_on(features: &FeatureToolchain, device: Device<B>) -> Result<Self, PackageError> {
        ...
        let (model, _) = load_model_package::<B, FeatherHubertEncoder<B>, _>(
            directory,
            &ModelDescription::feather_hubert(config.clone()),
            &device,
            |device| config.init::<B>(device),
        )?;
        Ok(Self {
    ...
    pub fn into_parts(self) -> (BurnFeatherHubertEncoder<B>, String) {
