...
impl TestGpu {
    ...
    fn graphics_api(&self) -> &str {
        self.context.graphics_api()
    }
}
...
fn gpu() -> TestGpu {
    ...
    let adapter = registry
        .resolve(GPU_BACKEND, None)
        .expect("a certified native GPU is required");
    assert_eq!(adapter.backend, GPU_BACKEND);
    assert!(adapter.certified);
    let (device, context) = open_gpu(registry, &adapter.id);
    TestGpu { device, context }
}
...
#[test]
#[ignore = "requires a certified GPU for this suite's backend (and CUDA toolkit for CUDA)"]
fn a_training_checkpoint_resumes_cpu_to_gpu_and_back_to_cpu() {
    let _guard = GPU_LOCK.lock().unwrap();
    on_step_stack("gpu-checkpoint-portability", || {
...
#[test]
#[ignore = "requires a certified GPU for this suite's backend (and CUDA toolkit for CUDA)"]
fn both_unets_train_all_three_modes_on_gpu() {
    let _guard = GPU_LOCK.lock().unwrap();
    on_step_stack("gpu-training-modes", || {
        let ctx = gpu();
        for mode in [
            TrainingMode::Baseline,
            ...
            for variant in [UnetVariant::OriginalUnet, UnetVariant::MobileOneUnet] {
                ...
                assert_eq!(result["backend"], GPU_NAME);
...
#[test]
#[ignore = "requires a certified GPU for this suite's backend (and CUDA toolkit for CUDA)"]
fn a_gpu_render_publishes_frames_and_reports_its_backend() {
    let _guard = GPU_LOCK.lock().unwrap();
    on_step_stack("gpu-render", || {
        let ctx = gpu();
...
fn feature_command_uses_the_selected_gpu_and_matches_cpu_output() {
    let _guard = GPU_LOCK.lock().unwrap();
    on_step_stack("gpu-feature-command", || {
        let root = tempfile::tempdir().unwrap();
        ...
        let adapter = registry().resolve(GPU_BACKEND, None).unwrap();
        let mut outputs = Vec::new();
        for backend in ["cpu", GPU_NAME] {
            ...
            let config = base.clone().with_compute_selection(
                Some(backend),
                (backend == GPU_NAME).then_some(adapter.id.as_str()),
            );
...
#[test]
#[ignore = "requires a certified GPU for this suite's backend and the committed SCRFD/PFLD artifacts"]
fn frame_models_load_and_infer_on_the_selected_gpu() {
    let _guard = GPU_LOCK.lock().unwrap();
    on_step_stack("gpu-frame-models", || {
        let ctx = gpu();
