...
fn malformed_and_unknown_frames_are_refused() {
    for bad in [
        ...
        r#"{"frame":"shutdown","data":{"protocol_version":4,"extra":true}}"#,
    ] {
        assert!(
            matches!(
...
fn client_and_server_frames_reject_unknown_outer_fields() {
    let client = r#"{"frame":"shutdown","data":{"protocol_version":4},"extra":true}"#;
    assert!(serde_json::from_str::<ClientFrame>(client).is_err());

    let server = r#"{"frame":"rejected","data":{"protocol_version":4,"reason":"no"},"extra":true}"#;
