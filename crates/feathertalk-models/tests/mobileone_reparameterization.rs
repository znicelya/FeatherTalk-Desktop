...
fn square_stride_keeps_the_existing_pfld_shape_contract() {
    ...
    let block = MobileOneBlock::new(4, 8, 3, 2, 1, 1, 2, false, &device);
...
fn anisotropic_stride_halves_only_width() {
    let device = Default::default();
    let block =
        MobileOneBlock::new_with_stride(4, 8, 3, [1, 2], 1, 1, 2, false, &device);
    let input = Tensor::zeros([1, 4, 12, 14], &device);
...
fn scaled_branch_rejects_non_centered_padding_during_construction() {
    let device = Default::default();
    let _block =
        MobileOneBlock::new_with_stride(4, 8, 3, [1, 2], 0, 1, 2, false, &device);
}

fn assert_reparameterized_close(block: MobileOneBlock, input: Tensor<4>) {
    let expected = block
        .forward(input.clone())
        .into_data()
        .try_to_vec::<f32>()
...
fn reparameterized_one_by_one_block_matches_training_graph() {
    let device = Default::default();
    let block = MobileOneBlock::new(4, 8, 1, 1, 0, 1, 2, false, &device);
...
fn reparameterized_three_by_three_block_with_skip_matches_training_graph() {
    let device = Default::default();
    let block = MobileOneBlock::new(4, 4, 3, 1, 1, 1, 2, false, &device);
    assert_reparameterized_close(block, Tensor::ones([1, 4, 8, 8], &device));
}
...
fn reparameterized_depthwise_block_matches_training_graph() {
    let device = Default::default();
    let block = MobileOneBlock::new(4, 4, 3, 2, 1, 4, 2, true, &device);
    assert_reparameterized_close(block, Tensor::ones([1, 4, 8, 8], &device));
}
