...
use feathertalk_models::feather_hubert::{BurnFeatherHubertEncoder, FeatherHubertConfig};

#[test]
fn cpu_adapter_encodes_finite_rows_and_supports_long_audio_stitching() {
    let device: burn::tensor::Device = Default::default();
    let mut encoder =
        BurnFeatherHubertEncoder::from_config(FeatherHubertConfig::parity_micro(), &device);
...
fn cpu_adapter_accepts_tensor_data_shape_contract() {
    ...
    let tensor = Tensor::<2>::from_data(TensorData::new(vec![0.0_f32; 400], [1, 400]), &device);
