...
impl PerceptualFeatureExtractor for PanicExtractor {
    fn forward(&self, _image: Tensor<4>) -> Tensor<4> {
...
fn temporal_components_match_hand_computed_pair_and_union_literals() {
    let device = Default::default();
    let prediction = Tensor::<5>::zeros([1, 2, 3, 4, 4], &device)
        .slice_fill([0..1, 1..2, 0..3, 0..2, 0..4], 1.0)
        .slice_fill([0..1, 1..2, 0..3, 2..4, 0..4], 3.0);
    let target = Tensor::<5>::zeros([1, 2, 3, 4, 4], &device);
    let mask = Tensor::<5>::zeros([1, 2, 1, 4, 4], &device)
...
fn invalid_masks_and_temporal_pair_lengths_are_rejected() {
    let device = Default::default();
    let image = Tensor::<4>::zeros([1, 3, 4, 4], &device);
    let target = Tensor::<4>::zeros([1, 3, 4, 4], &device);
    let bad_channels = Tensor::<4>::zeros([1, 2, 4, 4], &device);
    assert!(matches!(
        mouth_l1_loss(image.clone(), target.clone(), bad_channels),
        Err(TrainingError::InvalidInput(_))
    ));

    let bad_spatial = Tensor::<4>::zeros([1, 1, 3, 4], &device);
    assert!(matches!(
        mouth_l1_loss(image.clone(), target.clone(), bad_spatial),
        Err(TrainingError::InvalidInput(_))
    ));

    let bad_batch = Tensor::<4>::zeros([2, 1, 4, 4], &device);
    assert!(matches!(
        mouth_l1_loss(image.clone(), target.clone(), bad_batch),
        Err(TrainingError::InvalidInput(_))
    ));

    let pair_one = Tensor::<5>::zeros([1, 1, 3, 4, 4], &device);
    let pair_three = Tensor::<5>::zeros([1, 3, 3, 4, 4], &device);
    let mask_one = Tensor::<5>::zeros([1, 1, 1, 4, 4], &device);
    let mask_three = Tensor::<5>::zeros([1, 3, 1, 4, 4], &device);
    assert!(matches!(
        temporal_loss(
            &IdentityExtractor,
            pair_one.clone(),
...
fn target_shape_mismatch_is_rejected_before_extractor_invocation() {
    ...
    let target = Tensor::<4>::zeros([1, 3, 5, 4], &device);
...
fn all_three_loss_totals_propagate_prediction_gradients() {
    ...
    let prediction = Tensor::<4>::ones([1, 3, 4, 4], &device).require_grad();
    let target = Tensor::<4>::zeros([1, 3, 4, 4], &device);
    let mask = Tensor::<4>::ones([1, 1, 4, 4], &device);
    ...
    let prediction = Tensor::<5>::ones([1, 2, 3, 4, 4], &device).require_grad();
    let target = Tensor::<5>::zeros([1, 2, 3, 4, 4], &device);
    let mask = Tensor::<5>::ones([1, 2, 1, 4, 4], &device);
...
fn assert_breakdown_value(breakdown: &LossBreakdown, tensor: Tensor<1>, expected: f32) {
    ...
    assert!(breakdown.total.clone().into_scalar::<f32>().is_finite());
}
