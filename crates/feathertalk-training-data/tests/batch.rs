...
fn a_single_frame_batch_has_one_row_per_item() {
    ...
    let batch = stack_single_frame_batch(&items, &device).unwrap();
...
fn a_single_frame_batch_keeps_the_item_order() {
    ...
    let device = Default::default();
    let batch = stack_single_frame_batch(&items, &device).unwrap();
    let values = batch.image.into_data().try_to_vec::<f32>().unwrap();
...
fn a_temporal_batch_flattens_both_halves() {
    ...
    let device = Default::default();
    let batch = stack_temporal_batch(&items, &device).unwrap();
...
fn a_temporal_batch_is_sample_major() {
    ...
    let device = Default::default();
    let batch = stack_temporal_batch(&items, &device).unwrap();
    let values = batch.target.into_data().try_to_vec::<f32>().unwrap();
...
fn an_empty_batch_is_rejected() {
    let device = Default::default();
    let error = stack_single_frame_batch(&[], &device).unwrap_err();
...
fn a_batch_of_mixed_item_kinds_is_rejected() {
    ...
    let device = Default::default();
    let single = stack_single_frame_batch(&items, &device).unwrap_err();
    assert!(single.to_string().contains("item 1 is a temporal pair"));
    let temporal = stack_temporal_batch(&items, &device).unwrap_err();
