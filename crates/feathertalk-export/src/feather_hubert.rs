...
use feathertalk_models::feather_hubert::FeatherHubertEncoder;
...
pub fn build_feather_hubert_package(
    ...
    let (model, checkpoint) =
        load_feather_hubert_checkpoint(source_snapshot.path(), &device)?;
    ...
    let report = write_model_package_with_validation_hook::<FeatherHubertEncoder, _, _>(
        &package_request,
        &model,
        &device,
