...
//! - **Opset 11**: Added ceil_mode attribute
use derive_new::new;
...
pub struct AvgPool2dConfig {
    ...
    /// Whether to use ceil mode for output size calculation (opset 11+)
    pub ceil_mode: bool,
    /// Auto padding mode
    ...
    fn infer_types(
        ...
        for (key, value) in node.attrs.iter() {
            match key.as_str() {
                ...
                "ceil_mode" => {
                    // ceil_mode support requires opset 11+
                    let ceil_mode = value.clone().into_i64();
                    if ceil_mode != 0 && opset < 11 {
                        return Err(ProcessError::Custom(format!(
                            "AveragePool: ceil_mode requires opset 11+, got opset {}",
                            opset
                        )));
                    }
                }
...
mod tests {
    ...
    fn test_avg_pool2d_ceil_mode_opset_validation() {
        // Test that ceil_mode=1 with opset < 11 is rejected
        let node = create_test_node(vec![3, 3], vec![1, 1], vec![0, 0, 0, 0], 0, 1, None);
        let mut node = node;
        let processor = AvgPool2dProcessor;
        let prefs = OutputPreferences::new();
        ...
        if let Err(ProcessError::Custom(msg)) = result {
            assert!(msg.contains("ceil_mode requires opset 11+"));
        }
    }
